...
def display_error_results(query_errors):
    for i in range(len(query_errors)):
        print(
            "    %s -- %s   %s " %
            (query_errors[i][0],
             query_errors[i][1],
...
query_for_first_3_records = "SELECT count(*) \
    ...
    GROUP BY articles.title ORDER BY author_views DESC  limit 3"
...
query_for_most_popular_authors = "select  count(*) as author_views , \
authors.name from articles inner \
...
query_to_get_date_errors_morethan_1 = "select error_date, percentage from ( \
    ...
    percentage from (select substring(cast(log.time as text), 0, 11) \
    as error_date, \
    count(*) as requests from log \
    where status like '%4%' group by error_date) \
    as log_percentage group by error_date \
    order by percentage desc) as last_query \
    where percentage >= 1  "

get_famous_authors = pull_query_results_from_db(
    query_to_get_date_errors_morethan_1)
